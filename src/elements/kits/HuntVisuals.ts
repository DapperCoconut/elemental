...
/** A warm, friendly brown pup, reddened and swollen under a Blood Moon. */
...
export const PUP_MOON_SCALE = 1.4;

// ── Weak Points (mastery) ─────────────────────────────────────────────────

/** Half-width of the wedge. The kit measures hits against this too, so it lives here. */
export const WEAK_HALF_ANGLE = Math.PI / 6;   // 30° either side
const WEAK_INNER_R = 24;
const WEAK_OUTER_R = 54;

/** Everything `HuntFx.drawBeastling` needs about the pup that is not its position. */
export interface BeastlingPose {
  /** Radians. Only its sign matters — the whole rig mirrors rather than rotating. */
  heading: number;
  /** Stride phase, advanced by distance walked. Drives legs, bob and ear lag. */
  gait: number;
  tailPhase: number;
  earLag: number;
  /** Absolute timestamps, compared against the `t` passed in. */
  blinkUntil: number;
  lungeUntil: number;
  /** A grenade clamped in its jaws. */
  carrying: boolean;
}
...
export class HuntFx extends FxBase {
  ...
  /**
   * The Weak Points wedge: the red seam sweeping around a body, with its two crust edges, rim
   * ticks and the crosshair pip riding the middle of the slice — the thing you actually aim at.
   *
   * `pulse` is a 0–1 breath; `angle` is the wedge's centre, which the kit spins at 0.9 rad/s.
   */
  static drawWeakWedge(
    gfx: Phaser.GameObjects.Graphics, cx: number, cy: number, angle: number, pulse: number,
  ): void {
    const a0 = angle - WEAK_HALF_ANGLE;
    const a1 = angle + WEAK_HALF_ANGLE;
    gfx.clear();
    gfx.setPosition(0, 0);
    ...
    const px = cx + Math.cos(angle) * (WEAK_OUTER_R + 5);
    const py = cy + Math.sin(angle) * (WEAK_OUTER_R + 5);
  ...
  /**
   * The mastery beastling, drawn from scratch every frame: gait-bobbed legs, floppy ears that
   * lag the turn, a wagging tail, a blinking face, and a Blood Moon variant that is bigger,
   * redder, and dripping.
   *
   * Deliberately not routed through `tint` — the pup is a warm brown animal rather than an
   * element effect, and a skin palette recolouring it would read as a different creature.
   */
  static drawBeastling(
    g: Phaser.GameObjects.Graphics,
    x: number, y: number, t: number, moon: boolean, pose: BeastlingPose,
  ): void {
    g.clear();

    const s = (moon ? PUP_MOON_SCALE : 1) * (t < pose.lungeUntil ? 1.12 : 1);
    const flip = Math.cos(pose.heading) < 0 ? -1 : 1;
    const cx = x;
    const bob = Math.sin(pose.gait) * 1.4 * s;
    const cy = y + bob;
    ...
    g.fillEllipse(cx, y + 13 * s, 26 * s - bob, 7 * s);

    // Tail: four tapering segments curling up off the rump.
    const wag = Math.sin(pose.tailPhase) * 0.5;
    let tx = cx - 13 * s * flip;
    let ty = cy - 4 * s;
    ...
    for (const [ox, phase] of legPairs) {
      const swing = Math.sin(pose.gait * 2 + phase);
      g.fillStyle(coat, 1);
    ...
    const hy = cy - 6 * s + Math.sin(pose.gait) * 0.8 * s;
    ...
    drawEar(-1, pose.earLag * 0.6 - 0.22, 0x4e3018, 12, 7);
    ...
    drawEar(-3.5, pose.earLag, ear, 15, 8.5);
    ...
    if (pose.carrying) {
      ...
      g.fillStyle(0xffdd44, 0.7 + 0.3 * Math.sin(t / 70));
      g.fillCircle(gx, gy - 9 * s, 2 * s);
    }

    if (moon) {
      g.lineStyle(1.5, 0xcc2222, 0.35 + 0.2 * Math.sin(t / 220));
      g.strokeCircle(cx, cy, 24 * s);
      g.fillStyle(0xaa1111, 0.7);
      g.fillCircle(mx + 3 * s * flip, my + 6 * s + (t / 12 % 6), 1.6 * s);
      g.fillCircle(mx - 2 * s * flip, my + 5 * s + ((t / 15 + 3) % 6), 1.2 * s);
    }
  }
}

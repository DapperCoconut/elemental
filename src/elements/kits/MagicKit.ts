...
export class MagicKit {
  ...
  handleInput(
    ...
    // While wheel open — key nav, release detection
    if (this.grimoireMenuOpen || this.necronomiconMenuOpen) {
      ...
      if (Phaser.Input.Keyboard.JustUp(eKey) && isGrimoire) {
        ...
        this._spawnAimCountdown(() => {
          ...
          this._dispatchGrimoireWedge(pick, ptr.worldX, ptr.worldY, 'player');
      ...
      if (Phaser.Input.Keyboard.JustUp(qKey) && !isGrimoire) {
        ...
        this._spawnAimCountdown(() => {
          const ptr = this.api.scene.input.activePointer;
          this._dispatchNecronomiconWedge(pick, ptr.worldX, ptr.worldY, 'player');
  ...
  update(time: number, delta: number): void {
    ...
    for (let i = this.healOrbs.length - 1; i >= 0; i--) {
      ...
      if (Phaser.Math.Distance.Between(orb.x, orb.y, caster.x, caster.y) <= 24) {
        caster.heal(5);
        if (orb.owner === 'player') this.api.showFloatingText(caster.x, caster.y - 28, '+5 ✨', '#cc99ff');
        orb.sprite.destroy();
        this.healOrbs.splice(i, 1);
      }
    }
      ...
      if (targets.length > 0) {
        ...
        while (c.tickAccum >= c.tickInterval) {
          for (const t of targets) {
            ...
            this.api.spawnHitFlash(t.x, t.y, 0xff6600);
      ...
      if (time >= c.nextPulseAt && c.nextPulseAt > 0) {
        ...
        for (const t of targets) {
          ...
          if (c.pulseDmg > 0) {
            ...
            this.api.spawnHitFlash(t.x, t.y, 0x44aaff);
          }
        }
  ...
  private _dispatchGrimoireWedge(pick: number, tx: number, ty: number, owner: 'player' | 'npc'): void {
    switch (pick) {
      case 0: this.doFlameBurst(tx, ty, owner); break;
      case 1: this.doStormCloudSummon(tx, ty, owner); break;
      case 2: this.doVirulentThorns(tx, ty, owner); break;
      case 3: this.doCompressionBlast(tx, ty, owner); break;
      case 4: this.doGaiasGuidance(owner); break;
    }
  }

  private _dispatchNecronomiconWedge(pick: number, tx: number, ty: number, owner: 'player' | 'npc'): void {
    switch (pick) {
      case 0: this.doFlameBarrage(tx, ty, owner); break;
      case 1: this.doFinalDrench(tx, ty, owner); break;
      case 2: this.doThornPrison(tx, ty, owner); break;
      case 3: this.doTornadoBlast(tx, ty, owner); break;
      case 4: this.doGaiasRage(owner); break;
    }
  }
  ...
  npcCastGrimoireWedge(tx: number, ty: number): void {
    ...
    this._dispatchGrimoireWedge(pick, tx, ty, 'npc');
  }
  ...
  npcCastNecronomiconWedge(tx: number, ty: number): void {
    ...
    this._dispatchNecronomiconWedge(pick, tx, ty, 'npc');
  }
  ...
  doAnchorToggle(owner: 'player' | 'npc'): void {
    ...
    if (anchorRef === null) {
      ...
      if (owner === 'player') this.api.player.triggerCooldown('magic-anchor');
    }
  }

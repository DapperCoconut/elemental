...
import { CORRUPT_WORLDS } from './CorruptWorlds';
...
export const ABSTRACT_WORLDS: World[] = [
  ...
  { id: 'subterfuge',     name: 'Subterfuge',  emoji: '🕴️',  color: 0xcc2233, parentId: 'light', mapX: 920, mapY: 390, nodes: makeNodes('subterfuge') },
];
...
/**
 * Every campaign world in every realm, in map order: normal, then Abstract, then Corrupt.
 *
 * Anything that means "all campaign content" — world lookups, the cheat save's completion
 * pass — must derive from this rather than listing the realms itself, so a realm added
 * later is picked up everywhere instead of being silently skipped in one place.
 */
export const ALL_WORLDS: World[] = [...WORLDS, ...ABSTRACT_WORLDS, ...CORRUPT_WORLDS];

export function getAnyWorld(id: string): World | undefined {
  return ALL_WORLDS.find((w) => w.id === id);
}

/** Every numbered fight in the campaign — five per world, across all three realms. */
export const TOTAL_FIGHTS = ALL_WORLDS.length * 5;

/** Every Sovereign, plus the Amalgam — which is filed as a challenge but is not a world. */
export const TOTAL_CHALLENGES = ALL_WORLDS.length + 1;

...
export class GauntletElementSelectScene extends Phaser.Scene {
  ...
  private selectElement(elementId: string): void {
    ...
    if (this.hardMode) {
      // 7 regular fights: cycle through the pool (5 elements) to fill 7 slots
      const shuffled = [...pool].sort(() => Math.random() - 0.5);
      fightOrder = Array.from({ length: 7 }, (_, i) => shuffled[i % shuffled.length]);

      // Per-fight mutation arrays for fights 1-7, plus boss extras at index 7
      const regularCounts = GAUNTLET_HARD_MUTATION_COUNTS; // [2,2,2,2,2,3,4]
      const regularMutations = regularCounts.map((n) => pickN(n));
      const bossExtra = pickN(1); // 1 random alongside 'boss'
      fightMutations = [...regularMutations, bossExtra];

      difficulty0 = GAUNTLET_HARD_DIFFICULTY[0];
      mutations0 = fightMutations[0];
    } else {
      fightOrder = [...pool].sort(() => Math.random() - 0.5);
      fightMutations = Array.from({ length: 5 }, () => pickN(1));

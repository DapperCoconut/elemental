...
Steps:
1. Read `src/elements/kits/LightKit.ts` to understand the pattern — `[Element]ArenaApi` interface + `[Element]Kit` class with `reset()`, `update()`, `handleInput()`, and public accessors for any fields read outside the kit.
